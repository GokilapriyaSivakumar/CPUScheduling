...
function showFinalTable(input, output, outputDiv) {
    ...
    let avgTurnAroundTime = output.turnAroundTime.reduce((a, b) => a + b, 0) / output.turnAroundTime.length;
    let avgWaitingTime = output.waitingTime.reduce((a, b) => a + b, 0) / output.waitingTime.length;
    let avgResponseTime = output.responseTime.reduce((a, b) => a + b, 0) / output.responseTime.length;

    let avgValues = document.createElement("p");
    avgValues.innerHTML = `Average Turn Around Time: ${avgTurnAroundTime.toFixed(2)}<br>Average Waiting Time: ${avgWaitingTime.toFixed(2)}<br>Average Response Time: ${avgResponseTime.toFixed(2)}`;
    outputDiv.appendChild(avgValues);
}
